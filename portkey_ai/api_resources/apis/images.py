...
class Images(APIResource):
    ...
    def generate(self, prompt: str, **kwargs) -> ImageResponse:
        response = self.openai_client.with_raw_response.images.generate(prompt=prompt, **kwargs)
    ...
    def edit(self, prompt: str, image, **kwargs) -> ImageResponse:
        response = self.openai_client.with_raw_response.images.edit(prompt=prompt, image=image, **kwargs)
        data = ImageResponse(**json.loads(response.text))
        data._headers = response.headers
    ...
    def create_variation(self, image, **kwargs) -> ImageResponse:
        response = self.openai_client.with_raw_response.images.create_variation(image=image, **kwargs)
        data = ImageResponse(**json.loads(response.text))
        data._headers = response.headers
...
class AsyncImages(AsyncAPIResource):
    ...
    async def generate(self, prompt: str, **kwargs) -> ImageResponse:
        response = await self.openai_client.with_raw_response.images.generate(prompt=prompt, **kwargs)
        data = ImageResponse(**json.loads(response.text))
        data._headers = response.headers

...
from portkey_ai.api_resources.utils import FileDeleteResponse, FileDeleted, FileObject, FileList
...
class MainFiles(APIResource):
    ...
    def create(self, file, purpose, **kwargs) -> FileObject:
        response = self.openai_client.with_raw_response.files.create(file=file, purpose=purpose, **kwargs)
    ...
    def retrieve(self, file_id, **kwargs) -> FileObject:
        response = self.openai_client.with_raw_response.files.retrieve(file_id=file_id, **kwargs)
        data = FileObject(**json.loads(response.text))
        data._headers = response.headers
    ...
    def delete(self, file_id, **kwargs) -> FileDeleted:
        response = self.openai_client.with_raw_response.files.delete(file_id=file_id, **kwargs)
...
class AsyncMainFiles(AsyncAPIResource):
    ...
    async def create(self, file, purpose, **kwargs) -> FileObject:
        response = await self.openai_client.with_raw_response.files.create(file=file, purpose=purpose, **kwargs)
        data = FileObject(**json.loads(response.text))
        data._headers = response.headers
    ...
    async def retrieve(self, file_id, **kwargs) -> FileObject:
        response = await self.openai_client.with_raw_response.files.retrieve(file_id=file_id, **kwargs)
        data = FileObject(**json.loads(response.text))
        data._headers = response.headers
    ...
    async def delete(self, file_id, **kwargs) -> FileDeleted:
        response = await self.openai_client.with_raw_response.files.delete(file_id=file_id, **kwargs)
        data = FileDeleted(**json.loads(response.text))
        data._headers = response.headers
